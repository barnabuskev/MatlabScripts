...
%       use N=[N1,N2] for a 2D filter.
%
%       K-comment: N alters the sampling of the gaussian kernel. If N = 0
%       then vector f's elements are equivalent to standard deviations. E.g
%       if n = 7, f(4) is the standard normal pdf at the mean, f(1) is 3 sd
%       below the mean, f(2) is 2 sd's etc. As N -> 1 sampling of the
%       standard normal pdf gets less dense around the mean, so that if N is
%       close to 1 then the elements of f rapidly go to zero for the
%       elemens away from the centre element(s). E.g. a five element
%       coefficient vector = [0,0,1,0,0]
%       
%      
%
