function varargout = projectzeus(varargin)
...
% PROJECTZEUS MATLAB code for projectzeus.fig
%      PROJECTZEUS, by itself, creates a new PROJECTZEUS or raises the existing
%      singleton*.
%
%      H = PROJECTZEUS returns the handle to a new PROJECTZEUS or the handle to
%      the existing singleton*.
%
%      PROJECTZEUS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PROJECTZEUS.M with the given input arguments.
%
%      PROJECTZEUS('Property','Value',...) creates a new PROJECTZEUS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before projectzeus_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to projectzeus_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help projectzeus

% Last Modified by GUIDE v2.5 29-Sep-2014 16:34:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @projectzeus_OpeningFcn, ...
    'gui_OutputFcn',  @projectzeus_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function projectzeus_OpeningFcn(hObject, eventdata, handles, varargin)
...
ud.savefile = '';
% read wav files
cd('C:\Users\Researcher\Documents\MATLAB\ProjectZeus')
[ud.same,Fs] = audioread('same.wav');
[ud.notsame,Fs] = audioread('notsame.wav');
ud.fs = Fs;
set(hObject,'UserData',ud)
% Choose default command line output for projectzeus
...
function varargout = projectzeus_OutputFcn(hObject, eventdata, handles)
...
ud = get(fig_h,'UserData');
datcell(1,:) = {'subject','year','compares'};
