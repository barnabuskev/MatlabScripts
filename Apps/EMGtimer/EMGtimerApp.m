...
%     app.EMGtimerApp
  ...
  methods
    ...
    function startApp(obj)
        ...
        if nargout(@EMGsession) == 0  
            eval('EMGsession');
        else
			obj.AppHandle = eval('EMGsession');   
        end 
        % Increment the reference count by one    
        EMGtimerApp.refcount(obj.Increment);
