function varargout = EMGsetup(varargin)
% EMGSETUP M-file for EMGsetup.fig
%copyright Kevin Brownhill
%version 1 Feb 2003
%      EMGSETUP, by itself, creates a new EMGSETUP or raises the existing
%      singleton*.
%
%      H = EMGSETUP returns the handle to a new EMGSETUP or the handle to
%      the existing singleton*.
%
%      EMGSETUP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in EMGSETUP.M with the given input arguments.
%
%      EMGSETUP('Property','Value',...) creates a new EMGSETUP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before EMGsetup_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to EMGsetup_OpeningFcn via varargin.
%
%      *See GUI Options on GUIDE's Tools menu.  Choose "GUI allows only one
%      instance to run (singleton)".
%
% See also: GUIDE, GUIDATA, GUIHANDLES
% Edit the above text to modify the response to help EMGsetup
% Last Modified by GUIDE v2.5 17-Oct-2002 15:00:59
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @EMGsetup_OpeningFcn, ...
                   'gui_OutputFcn',  @EMGsetup_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before EMGsetup is made visible.
function EMGsetup_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to EMGsetup (see VARARGIN)
%
% Called as follows: calibstate = EMGsetup(calibstate,ai,channel,dg_axes)
...
function varargout = EMGsetup_OutputFcn(hObject, eventdata, handles)
...
if ~(handles.calibstate(1) & handles.calibstate(2)) %channel not calibrated
    if get(handles.high,'Value') %high button selected
        ...
        if handles.calibstate(1)
            ...
            beep
            msgbox('Calibrate High first!','Message from EMGSetup')
        end
    end
...
if ~handles.calibstate(1) | ~handles.calibstate(2)
    ...
    if handles.initIR ~= temp
        ...
        set(handles.info,'String',handles.high_str);
        set(handles.ready,'string','Ready');
        uiwait(msgbox('You will need to recalibrate','Message from EMGSetup','modal'));
        return
    end
    %calibrate channnel
...
% --- Executes on closing EMG window
function close_EMG
